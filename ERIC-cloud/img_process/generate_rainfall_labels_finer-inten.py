...
label_file = '../tamu_raindata_rainminute_annotated_5s.json'
...
def main():
   ...
   date_time = []
   for i in rainfall_label['img_name']:
      segs = i.split('-')
      time = dt.datetime(int(segs[0]),int(segs[1]),int(segs[2]),int(segs[3]),int(segs[4]),int(segs[5]))
      date_time.append(time)

   rainfall_label['DateTime'] = date_time
   rainfall_label.sort_values(by=['DateTime'], ascending = True, inplace=True)
